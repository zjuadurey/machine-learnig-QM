...
#             dis = min(math.sqrt(abs(xi-xk1)**2+abs(yi-yk1)**2), math.sqrt(abs(xi-xk2)**2+abs(yi-yk2)**2))
            ...
            #sfxy.append((cmath.exp(-complex(0,absx*2*3.14*float(sys.argv[4])+absy*2*3.14*float(sys.argv[5])))*(msxsx+msysy)).tolist())
            sfxy.append( (cmath.exp(-complex( 0, absx*2*3.14*float(sys.argv[4])*msxsx + absy*2*3.14*float(sys.argv[5])*msysy ))).tolist() )
            sitesfxy.append([i,k])

structure_factorx = nk.operator.LocalOperator(hi, sfxy, sitesfxy)
